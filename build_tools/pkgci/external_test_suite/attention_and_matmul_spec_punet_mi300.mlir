...
//===----------------------------------------------------------------------===//
//mlperf bs=9, cpd =2
transform.named_sequence @match_contraction_18432x1280x5120_i8xi8xi32(%arg0: !transform.any_op {transform.readonly}) -> (!transform.any_op, !transform.any_param) {
    %inputs, %outputs = transform.iree.match.cast_compatible_dag_from_root %arg0 {
    ^bb0(%arg1: tensor<18432x5120xi8>, %arg2: tensor<1280x5120xi8>, %arg3: tensor<18432x1280xi32>):
      %1 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction"]} ins(%arg1, %arg2 : tensor<18432x5120xi8>, tensor<1280x5120xi8>) outs(%arg3 : tensor<18432x1280xi32>) {
  ...
  transform.named_sequence @match_contraction_18432x1280x1280_i8xi8xi32(%arg0: !transform.any_op {transform.readonly}) -> (!transform.any_op, !transform.any_param) {
    %inputs, %outputs = transform.iree.match.cast_compatible_dag_from_root %arg0 {
    ^bb0(%arg1: tensor<18432x1280xi8>, %arg2: tensor<1280x1280xi8>, %arg3: tensor<18432x1280xi32>):
      %1 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction"]} ins(%arg1, %arg2 : tensor<18432x1280xi8>, tensor<1280x1280xi8>) outs(%arg3 : tensor<18432x1280xi32>) {
      ^bb0(%in: i8, %in_0: i8, %out: i32): 
        %2 = arith.extsi %in : i8 to i32
        %3 = arith.extsi %in_0 : i8 to i32 
        %4 = arith.muli %2, %3 : i32
  ...
  transform.named_sequence @match_contraction_73728x5120x640_i8xi8xi32(%arg0: !transform.any_op {transform.readonly}) -> (!transform.any_op, !transform.any_param) {
    %inputs, %outputs = transform.iree.match.cast_compatible_dag_from_root %arg0 {
    ^bb0(%arg1: tensor<73728x640xi8>, %arg2: tensor<5120x640xi8>, %arg3: tensor<73728x5120xi32>):
      %1 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction"]} ins(%arg1, %arg2 : tensor<73728x640xi8>, tensor<5120x640xi8>) outs(%arg3 : tensor<73728x5120xi32>) {
      ^bb0(%in: i8, %in_0: i8, %out: i32): 
        %2 = arith.extsi %in : i8 to i32
        %3 = arith.extsi %in_0 : i8 to i32 
        %4 = arith.muli %2, %3 : i32
  ...
  transform.named_sequence @match_contraction_18x1280x1024_i8xi8xi32(%arg0: !transform.any_op {transform.readonly}) -> (!transform.any_op, !transform.any_param) {
    %inputs, %outputs = transform.iree.match.cast_compatible_dag_from_root %arg0 {
    ^bb0(%arg1: tensor<18x1024x1280xi8>, %arg2: tensor<1280x1280xi8>, %arg3: tensor<18x1280x1024xi32>):
      %1 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%arg1, %arg2 : tensor<18x1024x1280xi8>, tensor<1280x1280xi8>) outs(%arg3 : tensor<18x1280x1024xi32>) {
      ^bb0(%in: i8, %in_0: i8, %out: i32): 
        %2 = arith.extsi %in : i8 to i32
        %3 = arith.extsi %in_0 : i8 to i32 
        %4 = arith.muli %2, %3 : i32
        %5 = arith.addi %out, %4 : i32
        linalg.yield %5 : i32
      } -> tensor<18x1280x1024xi32>
    } : (!transform.any_op) -> (!transform.any_value, !transform.any_value)
    %0 = transform.param.constant #iree_codegen.compilation_info<lowering_config = #iree_gpu.lowering_config<{mma_kind = #iree_gpu.mma_layout<MFMA_I32_16x16x32_I8>, promote_operands = [0, 1], reduction = [0, 0, 0, 128], subgroup_m_count = 4 : i64, subgroup_n_count = 2 : i64, workgroup = [1, 128, 128, 0]}>, translation_info = <pipeline = LLVMGPUVectorDistribute workgroup_size = [128, 4, 1] subgroup_size = 64, {gpu_pipeline_options = #iree_gpu.pipeline_options<prefetch_shared_memory = true>, llvm_func_attrs = {"amdgpu-waves-per-eu" = "2"}}>> -> !transform.any_param
    transform.yield %arg0, %0 : !transform.any_op, !transform.any_param
  }
//  transform.named_sequence @match_conv_2d_nhwc_hwcf_18x128x128x640x3x3x320_i8xi8xi32(%arg0: !transform.any_op {transform.readonly}) -> (!transform.any_op, !transform.any_param) {
//    %inputs, %outputs = transform.iree.match.cast_compatible_dag_from_root %arg0 {
...
//===----------------------------------------------------------------------===//
  ...
  transform.named_sequence @__kernel_config(%variant_op: !transform.any_op {transform.consumed}) {
    transform.foreach_match in %variant_op
        ...
        ,@match_contraction_18432x1280x1280_i8xi8xi32 -> @apply_op_config
        ,@match_contraction_73728x5120x640_i8xi8xi32 -> @apply_op_config
        ,@match_contraction_18x1280x1024_i8xi8xi32 -> @apply_op_config
       // ,@match_conv_2d_nhwc_hwcf_18x128x128x640x3x3x320_i8xi8xi32 -> @apply_op_config
       //mlperf bs=9, cpd=2
        // Broadcast rhs mmt.
